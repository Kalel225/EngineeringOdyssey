...
export const CockpitHeader: React.FC<CockpitHeaderProps> = ({
  ...
  return (
    <header className="hidden lg:flex fixed top-0 left-64 right-0 h-16 bg-[#0b0e16]/85 backdrop-blur-xl z-40 px-6 items-center justify-between border-b border-[#3a494b]/30 shadow-[0_1px_12px_rgba(0,0,0,0.4)] select-none">
      ...
      <div className="flex items-center gap-6">
        <div className="flex flex-col">
          ...
          <span className="font-mono text-sm font-bold text-[#00f2fe] tracking-wide">
            {formattedRA} / {formattedDEC}
          </span>
        </div>

        <div className="hidden xl:flex flex-col border-l border-[#3a494b]/30 pl-4">
          <span className="font-mono text-[10px] text-[#849495] uppercase tracking-wider">
            Velocity
          </span>
          <span className="font-mono text-sm font-semibold text-[#e1e2ee]">
            {velocityC.toFixed(3)}{' '}
            <span className="text-[11px] text-[#849495] font-normal">c</span>
          </span>
        </div>
        ...
        <div className="hidden xl:flex flex-col border-l border-[#3a494b]/30 pl-4">
          ...
          <span className="font-mono text-sm font-semibold text-[#e1e2ee]">
            {distAU.toFixed(3)}{' '}
            <span className="text-[11px] text-[#849495] font-normal">AU</span>
          </span>
        </div>

        <div className="flex items-center gap-2 bg-[#1d1f28] px-3 py-1 rounded border border-[#3a494b]/40">
      ...
      <div className="flex items-center gap-3">
        ...
        {isAuthenticated && officerUser ? (
          <div
            onClick={onOpenAuth}
            className="flex items-center gap-1.5 px-2.5 py-1 rounded bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 font-mono text-[11px] cursor-pointer hover:bg-emerald-500/25 transition-all"
          ...
          <button
            type="button"
            onClick={onOpenAuth}
            className="flex items-center gap-1.5 px-2.5 py-1 rounded bg-[#272a33] hover:bg-[#363943] text-amber-400 border border-amber-500/30 font-mono text-[11px] cursor-pointer transition-all"
        ...
        <button
          ...
          className="w-8 h-8 rounded-full bg-gradient-to-tr from-[#00dce6] to-[#6ff6ff] p-0.5 flex items-center justify-center cursor-pointer hover:ring-2 hover:ring-[#00f2fe] transition-all shadow-[0_0_8px_rgba(0,242,254,0.4)]"
